...
$(document).ready(function () {
    console.log('Soy el js load_message.js');
    ...
    // ---------------------------------------- Agregar Contacto ----------------------------------------
    $(document).on('click', '.contact-plus-icon', function (e) {
        e.stopPropagation();
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (data.status === 'success') {
                    ...
                    const errorMessage = data.data.message || 'No se pudo agregar el contacto.';
                    Swal.fire({
                        title: 'Advertencia!',
                        text: errorMessage,
                        icon: 'warning',
    ...
    const dropArea_Messages = document.getElementById('messages-container');
    window.addEventListener('dragover', (event) => event.preventDefault());
    window.addEventListener('drop', (event) => event.preventDefault());

    let draggedDefaultMessageId = null;
    let draggedProductId = null;
    let draggingType = null; // 'defaultMessage' o 'product'

    // ---------------------------------------- Eventos de Arrastre ----------------------------------------

    // Evento al iniciar el arrastre del mensaje por defecto
    $(document).on('dragstart', '.message-item', function (event) {
        draggedDefaultMessageId = $(this).data('message-id');
        draggedProductId = null; // Resetea el ID del producto
        draggingType = 'defaultMessage'; // Define el tipo actual de arrastre
    });

    // Evento al iniciar el arrastre del producto
    $(document).on('dragstart', '.product-item', function (event) {
        draggedProductId = $(this).data('product-id');
        draggedDefaultMessageId = null; // Resetea el ID del mensaje por defecto
        draggingType = 'product'; // Define el tipo actual de arrastre
    });

    // Evento al arrastrar sobre el área de mensajes
    dropArea_Messages.addEventListener('dragover', (event) => {
        event.preventDefault();
        dropArea_Messages.classList.add('drag-over');
    });

    // Quita el estilo de arrastre al salir del área de mensajes
    dropArea_Messages.addEventListener('dragleave', () => {
        dropArea_Messages.classList.remove('drag-over');
    });

    // Evento al soltar en el área de mensajes
    dropArea_Messages.addEventListener('drop', (event) => {
        event.preventDefault();
        dropArea_Messages.classList.remove('drag-over');
        ...
        // Procesar el elemento según el tipo de arrastre
        if (draggingType === 'defaultMessage' && draggedDefaultMessageId) {
            sendDefaultMessage(chatId, draggedDefaultMessageId);
        } else if (draggingType === 'product' && draggedProductId) {
            sendProductMessage(chatId, draggedProductId);
        }

        // Resetea las variables después de soltar
        draggedDefaultMessageId = null;
        draggedProductId = null;
        draggingType = null;
    });

    // ---------------------------------------- Función para enviar Mensaje por Defecto ----------------------------------------
    function sendDefaultMessage(chatId, defaultId) {
        const apiUrl = '/api/message/send-default-messages';
    ...
    // ---------------------------------------- Función para enviar Producto ----------------------------------------
    function sendProductMessage(chatId, productId) {
        const apiUrl = '/api/message/send-product';
        const csrfToken = $('meta[name="csrf-token"]').attr('content');
    ...
    $(document).on('click', '.message-options-btn', function (e) {
        ...
        var message_start = $(this).data('is-starred')
        ...
        if (window.currentChatStatus !== 'atendiendo' || window.currentAssignedUserId !== selectedSessionId) {
            ...
            if (existingMenu.length) {
                ...
                optionsMenuHtml += '<li><button class="forward-message" data-message-id="' + messageId + '" data-message-index="' + indexmessage + '" style="border: none; background: none; position: sticky;">Reenviar</button></li>';
                optionsMenuHtml += '<li><button class="important-message" data-message-id="' + messageId + '" data-is-starred="' + message_start + '" data-message-index="' + indexmessage + '" style="border: none; background: none; position: sticky;">' 
                    + (message_start ? 'No Destacar' : 'Destacar') 
                    + '</button></li>';
                if (messageElement.hasClass('from-me')) {
                    ...
                    optionsMenuHtml += '<li><button class="edit-message" data-message-id="' + messageId + '" data-message-index="' + indexmessage + '" style="border: none; background: none; position: sticky;">Editar</button></li>';
                }
                optionsMenuHtml += '</ul>';
                optionsMenuHtml += '</div>';
    ...
    // --------------------- Evento para reenviar mensajes ---------------------
    $(document).on('click', '.forward-btn', function () {
        // Habilita los checkboxes en todos los mensajes
        $('#message').each(function () {
            $(this).prepend('<input type="checkbox" class="message-checkbox" />');
        });
    
        // Muestra un botón para confirmar reenvío
        $('body').append('<button id="confirm-forward" style="position: fixed; bottom: 20px; right: 20px;">Confirmar Reenvío</button>');
    });

    // Capturar mensajes seleccionados
    ...
    // Confirmar y enviar los mensajes seleccionados
    $(document).on('click', '#confirm-forward', async function () {
        if (selectedMessages.length === 0) {
            alert('Por favor, selecciona al menos un mensaje para reenviar.');
            return;
        }
        ...
        try {
            const response = await fetch('/api/message/forward', {
                ...
                body: JSON.stringify({ message_ids: selectedMessages, to_chat_id: 'DESTINO_ID' })
            });
    ...
    $(document).on('click', '.edit-message', function (e) {
        ...
        var messageId = $(this).data('message-id');
        var messageElement = $(this).closest('.message');
        var messageContent = messageElement.find('.text-message-body').text();

        $('#editMessageModal .message-bubble').text(messageContent);
        $('#edit-message-input').val(messageContent);
    
        // Abrir el modal y colocar el contenido en el textarea
        $('#edit-message-input').val(messageContent).data('editing-message-id', messageId);
        $('#editMessageModal').fadeIn();
    });

    $('#confirm-edit-btn').on('click', function () {
        var messageId = $('#edit-message-input').data('editing-message-id');
        var newContent = $('#edit-message-input').val();
        const chat_ID = selectedChatId;
    
        // Verificar que estamos en modo de edición
        if (messageId) {
            $.ajax({
                url: '/api/message/edit', // URL del controlador de edición
                type: 'POST',
                data: JSON.stringify({ messageId, newContent }),
                contentType: 'application/json',
                success: function (response) {
                    if (response.status === 'success') {
                        // Actualizar el texto del mensaje en la interfaz
                        $('.message[data-message-id="' + messageId + '"]').find('.message-text').text(newContent);
                        $('#editMessageModal').fadeOut(); // Ocultar el modal
                        loadMessages(chat_ID)
                    } else {
                        Swal.fire({
                            title: 'Advertencia!',
                            text: 'El mensaje ya no se puede editar.',
                            icon: 'warning',
                            confirmButtonText: 'Aceptar'
                        });
                    }
                ...
                error: function () {
                    ...
                    Swal.fire({
                        title: 'Error!',
                        text: 'Error en la solicitud de edición.',
                        icon: 'error',
                        confirmButtonText: 'Aceptar'
                    });
                }
    ...
    $(document).on('click', function(e) {
        var modal = $('#editMessageModal .modal-content');
    ...
    // ---------------------------------------- Destacar Mensaje ----------------------------------------
    $(document).on('click', '.important-message', function (e) {
        e.stopPropagation();
        if (selectedChatId) {
    
            const chat_ID = selectedChatId;
            var messageId = $(this).data('message-id');
            var isStarred = $(this).data('is-starred');
    
            // Mostrar SweetAlert de confirmación
            Swal.fire({
                title: isStarred ? '¿Quitar destaque?' : '¿Destacar?',
                text: isStarred ? 'Este mensaje se desmarcará como importante.' : 'Este mensaje se marcará como importante.',
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#3085d6',
                cancelButtonColor: '#d33',
                confirmButtonText: isStarred ? 'Sí, quitar destaque' : 'Sí, destacar',
                cancelButtonText: 'Cancelar'
            }).then((result) => {
                if (result.isConfirmed) {
                    // Selecciona la URL de la API según el estado de "destacado"
                    var apiUrl = isStarred ? '/api/message/unmark_important' : '/api/message/mark_important';
                    $.ajax({
                        url: apiUrl,
                        type: 'POST',
                        headers: {
                        ...
                        success: function(response) {
                            if (response.status === 'success') {
                                loadMessages(chat_ID); 
                                Swal.fire({
                                    title: isStarred ? 'Quitar destaque' : '¡Destacado!',
                                    text: response.message,
                                    icon: 'success'
                                });
                            } else {
                                Swal.fire('¡Error!', 'No se pudo cambiar el estado de destaque del mensaje.', 'error');
                            }
                        },
                        error: function() {
                            Swal.fire('Error', 'No se pudo cambiar el estado de destaque del mensaje.', 'error');
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data && data.status === 'success') {
                    ...
                    // Variable para controlar la fecha actual y mostrar el indicador adecuado
                    ...
                    data.messages.forEach(function(msg, index) {
                        ...
                        // Comparación de fechas para insertar "Hoy", "Ayer" o fecha específica
                        var currentDate = new Date();
                        var displayDate = '';

                        if (isSameDay(originalDate, currentDate)) {
                            displayDate = 'Hoy';
                        } else if (isSameDay(originalDate, new Date(currentDate.setDate(currentDate.getDate() - 1)))) {
                            displayDate = 'Ayer';
                        } else {
                            displayDate = originalDate.toLocaleDateString();
                        }

                        // Solo agregar el indicador de fecha si es diferente al último mostrado
                        if (displayDate !== lastDisplayedDate) {
                            if (lastDisplayedDate !== null) {
                                messagesHtml += '</div>'; // Cierra el bloque anterior si existía
                            }
                        ...
                        var forwardedHtml = '';
                        if (msg.is_forwarded) {
                            forwardedHtml = '<div class="forwarded-message-label"><i class="fa fa-share" aria-hidden="true"></i> Reenviado</div>';
                        }
                        
                        // Indicador de destacado
                        var starredHtml = '';
                        if (msg.is_starred) {
                            starredHtml = '<div class="starred-message-icon"><i class="fa fa-star"></i></div>';
                        }
                        ...
                        messagesHtml += '<button class="message-options-btn" data-message-id="' + id_message + '" data-message-index="' + messageId + '" data-message-type="' + msg.type + '" data-is-starred="' + msg.is_starred + '" ><i class="fa fa-chevron-down"></i></button>';
    
                        messagesHtml += forwardedHtml;
                        messagesHtml += starredHtml;
                        ...
                        switch (msg.type) {
                            ...
                            case 'revoked':
                                ...
                                break;
                            default:
                                messagesHtml += '<p class="message-body">' + quotedMessageHtml + '<span class="text-message-body">' + (msg.body|| 'Sin contenido') + '</span></p>';
                                break;
                        }
                        messagesHtml += '</div>';
                    });

                    // Cierra el último bloque de fecha si existía
                    if (lastDisplayedDate !== null) {
                        messagesHtml += '</div>';
                    }
...
// Cargar imágenes en el arreglo y generar miniaturas
function loadImages() {
    images = [];
    $('#lightbox-thumbnails').empty(); // Limpiar miniaturas previas
    ...
    $('.message-media-img').each(function (index, img) {
        ...
        // Crear miniatura y agregarla al contenedor
        var $thumbnail = $('<img>')
...
// Función para descargar el sticker actual
function downloadImage() {
    if (currentIndex >= 0 && currentIndex < images.length) {
        var link = document.createElement('a');
        link.href = images[currentIndex].src;
        link.download = 'imageOdoo.png'; 
...
$('#lightbox-close').on('click', function () {
    $('#lightbox_messages_whatsapp').fadeOut();
});
...
$('#lightbox-download').on('click', function () {
    downloadImage();
});

// Navegación entre imágenes con botones
...
// Cargar stickers en el arreglo
function loadStickers() {
    stickers = [];

    $('.message-media-sticker').each(function (index, sticker) {
        var $message = $(sticker).closest('.message');
        var from = $message.hasClass('from-me') ? 'Tú' : $('.chat-banner .chat-name').text();
...
// Mostrar el sticker en el lightbox
function showSticker(index) {
    if (index >= 0 && index < stickers.length) {
        currentStickerIndex = index;
        $('#lightbox-sticker-img').attr('src', stickers[index].src);
        $('#lightbox-sticker-info').text(stickers[index].from + ' - ' + stickers[index].timestamp);
        $('#lightbox-sticker').fadeIn();
    }
}
...
function downloadSticker() {
    if (currentStickerIndex >= 0 && currentStickerIndex < stickers.length) {
        var link = document.createElement('a');
        link.href = stickers[currentStickerIndex].src;
        link.download = 'stickerOdoo.webp'; // Nombre del archivo descargado
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
    }
...
$(document).on('click', '.message-media-sticker', function () {
    loadStickers();
    var clickedSrc = $(this).attr('src');
    currentStickerIndex = stickers.findIndex(sticker => sticker.src === clickedSrc);
    showSticker(currentStickerIndex);
});

$('#lightbox-sticker-close').on('click', function () {
    $('#lightbox_stickers_whatsapp').fadeOut();
});

$('#lightbox-sticker').on('click', function (e) {
    if (e.target === this) {
        $(this).fadeOut();
    }
});

$('#lightbox-sticker-download').on('click', function () {
    downloadSticker();
});
