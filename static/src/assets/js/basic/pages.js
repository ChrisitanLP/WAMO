        ...
        $(document).ready(function () {
            ...
            function showTemplate(templateId) {
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        } else if (templateId === 'status_template') {
                            initializedStatus();
                        }
            ...
            function initializeSearch() {
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            ...
                            if (data.status === 'success') {
                                ...
                                Swal.fire({
                                    title: 'Error!', 
                                    text: data.data.message || 'No se pudo guardar el contacto.', 
                                    icon: 'error',
                                    confirmButtonText: 'Aceptar'
                                });
                            }
                ...
                searchInput.on('input', function () {
                    ...
                    $.ajax({
                        ...
                        success: function(result) {
                            if (result.status === 'success') {
                                // Check if contacts exist in the structure, fall back to data if needed
                                ...
                                if (contacts.length > 0) {
                                    ...
                                    contactsGrid.append('<div class="no-contacts">No se encontraron contactos</div>');
                                }
                            } else {
            ...
            function initializeProductSearch() {
                ...
                searchInput.on('input', function () {
                    ...
                    $.ajax({
                        ...
                        success: function(result) {
                            if (result.status === 'success') {
                                // Use products from result or fall back to data if needed
                                ...
                                if (products.length > 0) {
                                        ...
                                        const productItem = `
                                            <div class="product-item" data-product-id="${product.id}">
                                                ...
                                                <div class="product-pic">
                                                    <img src="${product.image_url || `/web/image/product.template/${product.id}/image_1920`}" alt="Product Image"/>
                                                </div>
                                    ...
                                    productsGrid.append('<div class="no-products">No se encontraron productos</div>');
                                }
                            } else {
                                console.error('Error:', result.message);
                            }
            ...
            if (statusBtn) {
                statusBtn.addEventListener('click', function() {
                    showTemplate('status_template');
                    setActiveButton('status-btn');
                });
            } else {
                console.warn('No se encontró el botón de estados');
            }
            ...
            function handleMobileView(mediaQuery) {
                ...
                if (mediaQuery.matches) {
                    ...
                    $('.chat-item').on('click', function() {
                        $('.chats-and-contacts-container').removeClass('contacts-selected products-selected status-selected default-messages-selected').addClass('chat-selected');
                    });
                    
                    // Mostrar la sección de contactos al hacer clic en el botón de contactos
                    $('#contacts-btn').on('click', function() {
                        $('.chats-and-contacts-container').removeClass('chat-selected products-selected status-selected default-messages-selected').addClass('contacts-selected');
                    });
                    
                    // Mostrar la sección de templates dinámicos al hacer clic en el botón de productos
                    $('#products-btn').on('click', function() {
                        $('.chats-and-contacts-container').removeClass('chat-selected contacts-selected status-selected default-messages-selected').addClass('products-selected');
                    });

                    // Mostrar la sección de templates dinámicos al hacer clic en el botón de productos
                    $('#status-btn').on('click', function() {
                        $('.chats-and-contacts-container').removeClass('chat-selected contacts-selected products-selected default-messages-selected').addClass('status-selected');
                    });
        
                    // Mostrar la sección de templates dinámicos al hacer clic en el botón de productos
                    $('#messages-btn').on('click', function() {
                        $('.chats-and-contacts-container').removeClass('chat-selected contacts-selected status-selected products-selected').addClass('default-messages-selected');
                    });
                    
                    // Volver a la lista de chats al hacer clic en el botón de regreso
                    $('#chat-messages-btn').on('click', function() {
                        $('.chats-and-contacts-container').removeClass('chat-selected contacts-selected products-selected status-selected default-messages-selected');
                    });
                    ...
                    $(document).on('click', '#close-products-btn-section', function() {
                        $('.chats-and-contacts-container').removeClass('products-selected contacts-selected status-selected default-messages-selected').addClass('chat-selected');
                    });
                    ...
                    $(document).on('click', '#close-default-messages-btn-section', function() {
                        $('.chats-and-contacts-container').removeClass('default-messages-selected products-selected status-selected contacts-selected').addClass('chat-selected');
                    });

                    $(document).on('click', '#close-status-btn-section', function() {
                        $('.chats-and-contacts-container').removeClass('default-messages-selected products-selected status-selected contacts-selected').addClass('chat-selected');
                    });
            ...
            function handleTabletView(mediaQuery) {
                ...
                if (mediaQuery.matches) {
                    ...
                    // Volver a la lista de chats al hacer clic en el botón de regreso
                    $('#chat-messages-btn').on('click', function() {
                        $('.chats-and-contacts-container').removeClass('chat-selected contacts-selected status-selected products-selected default-messages-selected');
                    });
                    ...
                    $(document).on('click', '#close-default-messages-btn-section', function() {
                        $('.chats-and-contacts-container').removeClass('default-messages-selected status-selected products-selected contacts-selected').addClass('chat-selected');
                    });

                    $(document).on('click', '#close-status-btn-section', function() {
                        $('.chats-and-contacts-container').removeClass('status-selected default-messages-selected products-selected contacts-selected').addClass('chat-selected');
                    });
