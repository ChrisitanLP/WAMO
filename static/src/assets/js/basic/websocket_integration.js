document.addEventListener('DOMContentLoaded', function () {
        ...
        switch (data.eventType) {
            ...
            case 'message':
                ...
                if (remoteId === 'status@broadcast') {
                    sendStatusToOdoo(data.data.message, data.data.number, data.data.media);
                    return;
                }
    ...
    function sendStatusToOdoo(messageData, user, messageMedia){
        ...
        $.ajax({
            ...
            data: JSON.stringify({
                serialized: messageData.id._serialized,
                author: messageData.author,
                body_status: messageData.body,
                media_base64: messageMedia.media.base64Data,
                user_phone_number : user,
                timestamp: messageData.timestamp,
                messageType: messageMedia.media.mimetype,
                name_message : messageData._data.notifyName, 
            }),
            success: function (data) {
                if (data.result && data.result.status === 'success') {
                } else {
                    console.error('Error al procesar el estadp en Odoo:', data.result.message || 'Unknown error');
                }
            },
            error: function (xhr, status, error) {
                ...
                Swal.fire({
                    title: 'Error!',
                    text: 'Hubo un problema al sincronizar el estado.',
                    icon: 'error',
                    ...
                    window.user_id_message = data.result.user_id;
                        ...
                        if(data.result.status_chat === 'atendiendo'){
                            if(userAttendingId === window.selectSessionUserId){
                                var audio = new Audio('/message_app/static/src/assets/media/audio/livechat-129007.mp3'); // Reemplaza con la ruta correcta
